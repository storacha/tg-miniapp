...
export const encodeAndEncrypt = async <Code extends number, T>(
  codec: BlockEncoder<Code, T>,
  cipher: Encrypter,
  value: T
): Promise<EncryptedTaggedByteView<T>> => {
  const bytes = codec.encode(value)
  const link = createLink<T, number, typeof identity.code>(
    codec.code,
    Digest.create(identity.code, bytes)
  )
  return cipher.encrypt(link.bytes)
}

export const createEncodeAndEncryptStream = <Code extends number, T>(
  codec: BlockEncoder<Code, T>,
  cipher: Encrypter,
  data: T
) =>
  createFileEncoderStream({
    stream: () =>
      new ReadableStream<EncryptedTaggedByteView<T>>({
        async pull(controller) {
          controller.enqueue(
            await encodeAndEncrypt<Code, T>(codec, cipher, data)
          )
          controller.close()
        },
      }),
...
export const decryptAndDecode = async <Code extends number, T>(
  cipher: Decrypter,
  codec: BlockDecoder<Code, T>,
  bytes: EncryptedTaggedByteView<T>
): Promise<T> => {
  ...
  if (link.multihash.code !== identity.code) {
    throw new Error(
      `invalid hasher: ${link.multihash.code} != ${identity.code}`
    )
  }
